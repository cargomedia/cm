...
function smarty_function_location(array $params, Smarty_Internal_Template $template) {
	/** @var CM_Location $location */
	$location = $params['location'];
	$showLink = !empty($params['link']);
	$parts = array();

	if ($city = $location->get(CM_Location::LEVEL_CITY)) {
		$part = $city->getName();
		if ($showLink) {
	...
	if ($state = $location->get(CM_Location::LEVEL_STATE)) {
		$part = $state->getName();
		if ($showLink) {
	...
	if ($country = $location->get(CM_Location::LEVEL_COUNTRY)) {
	...
	if (3 == count($parts)) {
		if ('US' == $location->getAbbreviation(CM_Location::LEVEL_COUNTRY)) {
			unset($parts[2]);
		} else {
			unset($parts[1]);
		}
