...
if (!$isInnoDb('cm_stream_publish')) {
    ...
    CM_Db_Db::exec("ALTER TABLE  `cm_stream_publish` ADD CONSTRAINT `cm_stream_publish-cm_streamChannel` FOREIGN KEY (  `channelId` ) REFERENCES  `skadate`.`cm_streamChannel` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT;");
}
if (!$isInnoDb('cm_stream_subscribe')) {
    ...
    CM_Db_Db::exec("ALTER TABLE  `cm_stream_subscribe` ADD CONSTRAINT `cm_stream_subscribe-cm_streamChannel` FOREIGN KEY (  `channelId` ) REFERENCES  `skadate`.`cm_streamChannel` (`id`) ON DELETE RESTRICT ON UPDATE RESTRICT ;");
}
