...
if(!CM_Db_Db::existsColumn('cm_locationCity', 'coordinates')) {
	CM_Db_Db::exec('ALTER TABLE `cm_locationCity` ADD COLUMN `coordinates` point default POINT(0, 0)');
	CM_Db_Db::exec('UPDATE `cm_locationCity` SET `coordinates` = POINT( lat, lon )');
	CM_Db_Db::exec("UPDATE `cm_locationCity` SET `coordinates` = POINT( 0, 0) WHERE `coordinates` = ''");
	CM_Db_Db::exec('CREATE SPATIAL INDEX `coordinates_spatial` ON cm_tmp_location(`coordinates`)');
}
