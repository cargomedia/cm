...
class CM_App_Cli extends CM_Cli_Runnable_Abstract {
    ...
    public function setConfig($filename, $configJson, $merge = null) {
        ...
        $configFile->truncate();
        $configFile->appendLine('<?php');
        $configFile->appendLine('// This is autogenerated config file. You should not change it manually.');
        $configFile->appendLine();
        $configFile->appendLine('return function (CM_Config_Node $config) {');
        $configFile->appendLine($indent($configStr));
        $configFile->appendLine('};');
