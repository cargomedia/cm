...
class CM_Response_Resource_Javascript_Library extends CM_Response_Resource_Javascript_Abstract {
	...
	protected function _process() {
		if ($this->getRequest()->getPath() === '/library.js') {
			...
			if ($skipLibraries) {
				$this->_setResource(new CM_App_Resource_Javascript_Internal($this->getSite()));
			} else {
				$this->_setResource(new CM_App_Resource_Javascript_Library($this->getSite()));
			}
			return;
		}
		if ($this->getRequest()->getPathPart(0) === 'translations') {
			...
			$this->_setResource(new CM_App_Resource_Javascript_Translations($language));
			return;
		}
