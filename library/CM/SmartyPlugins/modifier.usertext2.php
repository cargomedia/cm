...
function smarty_modifier_usertext($text, $mode = null, $lengthMax = null, $visibleTags = null) {
	...
	// emoji
	$string = preg_replace('/:(.*):/U','<span class="emoji $1"></span>',$string);

	// markdown
	$markdownParser = new Michelf\Markdown();
	$string = $markdownParser::defaultTransform($string);


	if ($mode=='plain') {
		//$string = strip_tags($string, '<p><span>');
	}


	$string = '<div class="newUserText">'.$string.'</div>';
	return $string;
