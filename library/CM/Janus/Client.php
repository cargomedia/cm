...
class CM_Janus_Client {
    ...
    public function synchronize() {
        ...
        $status = array();
        foreach ($this->_servers as $serverId => $janusServer) {
            $singleStatus = CM_Params::decode($this->_fetchStatus($janusServer['privateIp']), true);
            foreach ($singleStatus as $streamName => $publish) {
                $publish['serverId'] = $serverId;
                $publish['serverHost'] = $janusServer['privateIp'];
    ...
    /**
     * @param string $janusHost
     * @return string
     */
    protected function _fetchStatus($janusHost) {
        return CM_Util::getContents('http://' . $janusHost . ':' . $this->_config['httpPort'] . '/status');
    }
