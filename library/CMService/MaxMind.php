...
class CMService_MaxMind extends CM_Class_Abstract {
    ...
    protected $_outputStream, $_errorStream;
    ...
    public function outdated() {
        $this->_writeErrorLine('Updating locations database…');
    ...
    public function upgrade() {
        ...
        $this->_writeErrorLine('Updating search index…');
    ...
    protected function _compareCountryLists() {
        $this->_writeErrorLine('Comparing both country listings…');
    ...
    protected function _compareRegionLists() {
        $this->_writeErrorLine('Comparing both region listings…');
    ...
    protected function _compareLocationTrees() {
        $this->_writeErrorLine('Comparing both location trees…');
    ...
    protected function _getCountryData() {
        $this->_writeErrorLine('Downloading new country listing…');
        ...
        $this->_writeErrorLine('Reading new country listing…');
        $countryData = array();
    ...
    protected function _getIpData() {
        $this->_writeErrorLine('Reading new IP blocks…');
    ...
    protected function _getLocationData() {
        $this->_writeErrorLine('Reading new location tree…');
        if (null !== $this->_geoIpFile) {
    ...
    protected function _getRegionData() {
        $this->_writeErrorLine('Downloading new region listing…');
        ...
        $this->_writeErrorLine('Reading new region listing…');
        $regionData = array();
        $rows = preg_split('#[\r\n]++#', $regionsContents);
        foreach ($rows as $row) {
            $csv = str_getcsv(trim($row));
    ...
    protected function _readCountryListOld() {
        $this->_writeErrorLine('Reading old country listing…');
    ...
    protected function _readRegionListOld() {
        $this->_writeErrorLine('Reading old region listing…');
    ...
    protected function _readLocationTreeOld() {
        $this->_writeErrorLine('Reading old location tree…');
    ...
    protected function _updateIpBlocks() {
        ...
        $this->_writeErrorLine('Checking overlapping of IP blocks…');
        ksort($ipBlockList);
    ...
    protected function _upgradeCountryList() {
        $this->_writeErrorLine('Updating countries database…');
    ...
    protected function _upgradeRegionList() {
        $this->_writeErrorLine('Updating regions database…');
    ...
    protected function _upgradeCityList() {
        $this->_writeErrorLine('Updating cities database…');
    ...
    protected function _upgradeZipCodeList() {
        $this->_writeErrorLine('Updating zip codes database…');
    ...
    protected function _upgradeIpBlocks() {
        ...
        $this->_writeErrorLine('Updating IP blocks database…');
     ...
     * @codeCoverageIgnore
     */
    protected function _writeErrorLine($message) {
     ...
     * @codeCoverageIgnore
     */
    protected function _writeOutputLine($message) {
        if ($this->_outputStream) {
            $this->_outputStream->writeln($message);
        }
    }
    ...
    private function _printInfoList($infoList, $symbol = null) {
        ...
        foreach ($infoList as $info => $items) {
            ...
            foreach ($items as $item) {
                $this->_writeOutputLine(' ' . $symbol . ' ' . $item);
            }
    ...
    private function _printProgressCounter($item, $count) {
        $item = (int) $item;
        $count = (int) $count;
        $this->_writeError("\r$item/$count (" . round($item / $count * 100) . "%)");
        if ($item == $count) {
            $this->_writeErrorLine('');
        }
    }
    ...
    private function _printSeparator() {
        $this->_writeOutputLine('');
        $this->_writeOutputLine(str_repeat(' *', 10));
        $this->_writeOutputLine('');
    }
