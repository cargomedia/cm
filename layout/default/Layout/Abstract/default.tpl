...
<html {if $render->getLanguage()}lang="{$render->getLanguage()->getAbbreviation()}"{/if} class="{block name='html-class'}{/block}" {if (isset($webFontLoaderConfig) && null !== $webFontLoaderConfig)}data-web-font-loader='{$webFontLoaderConfig}'{/if}>
