...
class smarty_function_resourceUrlTest extends CMTest_TestCase {
    ...
    public function testRenderCrossSite() {
        ...
        $template = $smarty->createTemplate('string:');
        $template->assignGlobal('render', $render);
        $this->assertSame(sprintf('http://cdn.other.com/site-%s/version-%s/layout/foo', $siteOther->getId(), CM_App::getInstance()->getDeployVersion()),
        ...
        $this->assertSame(sprintf('http://www.example.com/site-%s/version-%s/layout/foo', $siteOther->getId(), CM_App::getInstance()->getDeployVersion()),
