...
class CM_Mail_MailerTest extends CMTest_TestCase {
    ...
    public function testSendWarning() {
        ...
        $errorMethod = $logger->mockMethod('error');
        $warningMethod = $logger->mockMethod('warning')->set(function ($message, CM_Log_Context $context = null) {
            $this->assertSame('Failed to send email to some recipients', $message);
            $this->assertSame([
                'message'          => [
                    'subject' => 'foo',
                ...
                'failedRecipients' => ['foo@example.com'],
            ], $context->getExtra());
        });
        ...
        /** @var CM_Exception_Invalid $exception */
        $this->assertSame(1, $sendMethod->getCallCount());
        $this->assertSame(1, $warningMethod->getCallCount());
        $this->assertSame(0, $errorMethod->getCallCount());
    }
