...
class CM_FormField_GeoPointTest extends CMTest_TestCase {
    ...
    public function testValidate() {
        ...
        $value = $field->validate($environment, array('latitude' => -30.2, 'longitude' => -122.2), $response);
        ...
        $value = $field->validate($environment, array('latitude' => 0, 'longitude' => 0), $response);
        $this->assertSame(0.0, $value->getLatitude());
        $this->assertSame(0.0, $value->getLongitude());

        try {
            $field->validate($environment, array('latitude' => 300, 'longitude' => 20), $response);
        ...
        try {
            $field->validate($environment, array('latitude' => -30.2, 'longitude' => 300), $response);
        ...
        try {
            $field->validate($environment, array('latitude' => 30), $response);
        ...
        try {
            $field->validate($environment, array('latitude' => 'foo', 'longitude' => 30), $response);
        ...
        try {
            $field->validate($environment, array('latitude' => 30, 'longitude' => 'foo'), $response);
