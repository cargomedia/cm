...
class CM_MessageStream_Adapter_SocketRedisTest extends CMTest_TestCase {
    ...
    public function testOnRedisMessageSubscribe() {
        ...
        $this->assertSameTime($streamChannel->canSubscribe($streamSubscribe, null, time()), $streamSubscribe->getAllowedUntil());
        ...
        $this->assertSameTime($timeStarted, $streamSubscribe->getStart());
        $this->assertSameTime($streamChannel->canSubscribe($streamSubscribe, null, $timeStarted), $streamSubscribe->getAllowedUntil());
    }
    ...
    private function _testSynchronize($status) {
        ...
        foreach ($status as $channel => $channelData) {
            ...
            foreach ($channelData['subscribers'] as $clientKey => $subscriberData) {
                ...
                $this->assertSameTime($streamChannel->canSubscribe($subscribe, null, time()), $subscribe->getAllowedUntil());
            }
        }
    }
}
